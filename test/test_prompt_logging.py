...
class TestPromptLogging:
    ...
    def mock_models(self):
        ...
        def mock_apply_chat_template(messages, add_generation_prompt=True, tokenize=False):
            formatted = ""
            for msg in messages:
    ...
    async def test_stream_generate_prompt_logging(self, mock_models, log_capture):
        ...
        test_prompt = "これはテスト用のプロンプトです。少し長いプロンプトをテストします。"
        
        # ストリーミング生成を実行
        response = ""
        async for token in llm_manager.generate_stream(test_prompt):
            response += token
    ...
    def test_generate_rag_query_prompt_logging(self, mock_models, log_capture):
        ...
        messages = [
            {"role": "user", "content": "Pythonのデータ型について詳しく説明してください"}
        ]
